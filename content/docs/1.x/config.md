...
The Config gives your module access to **key-value configuration** from your project's config files. It's accessible from the [Factory](/docs/1.x/factory) out of the box, so you can pass config values directly into your domain objects at creation time.

::: info
The examples below use PHP config files by default (`config/*.php`). See [Bootstrap > Application Config](/docs/1.x/bootstrap#application-config) for other formats and custom readers.
:::
