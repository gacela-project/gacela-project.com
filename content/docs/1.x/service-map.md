...
`#[ServiceMap]` replaces the older DocBlock `@method` dance. Both work; the attribute is more discoverable, survives PHPStan/Psalm out of the box (with [the bundled configs](/docs/1.x/static-analysis)), and is friendlier to static tooling.
...
If you are authoring a class managed by another container (Symfony, Laravel), prefer constructor injection with [`#[Inject]`](/docs/1.x/inject). `#[ServiceMap]` is targeted at classes instantiated outside of Gacela where constructor injection is not practical.
...
- Static analysis may still flag magic calls; use the [PHPStan / Psalm configs](/docs/1.x/static-analysis) shipped with Gacela to suppress them.
