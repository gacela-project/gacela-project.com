...
Scope which directories this (and `debug:modules`, `cache:warm`, `doctor`) scans with [`setAppModulePaths()`](/docs/1.x/bootstrap#application-module-paths).
...
- Parameters annotated with [`#[Inject]`](/docs/1.x/inject) show up tagged `inject`, with the override concrete rendered inline when present.
