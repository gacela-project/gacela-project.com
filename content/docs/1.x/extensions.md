...
The class must be invokable, and it has autoload capabilities: all dependencies will be resolved automatically as soon as you have defined them using [bindings](/docs/1.x/bindings). The same applies to the callable arguments above.
...
Register a per-module health check. All registered checks are aggregated by the `doctor` command and the `HealthChecker`. See the full [Module health checks](/docs/1.x/health-checks) page.
