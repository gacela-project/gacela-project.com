...
- [Testing](/docs/1.x/testing) — `GacelaTestCase` records these events and turns them into assertions (`assertServiceResolved()`, `assertBindingRegistered()`).
- [Module Customization](/docs/1.x/customization#listening-to-internal-events) — where listeners fit among the other `gacela.php` hooks.
- [Bootstrap](/docs/1.x/bootstrap) — the full `GacelaConfig` surface.
